<?php
/**
 * Limpa e normaliza dados de filiação 2020
 * Fonte: Google Forms "Ficha Filiação 2020"
 *
 * Colunas:
 * 0: # Internacional (número ou vazio)
 * 1: Carimbo de data/hora
 * 2: Endereço de e-mail
 * 3: Nome completo
 * 4: Título de Formação
 * 5: Cargo na rede Docomomo Brasil
 * 6: Endereço 1 (Logradouro, Bairro, CEP)
 * 7: Endereço 2 (Cidade, Estado)
 * 8: Número de Telefone (com DDD)
 * 9: Categoria de Inscrição
 *
 * OBS: Este arquivo parece ter apenas filiados "Pleno Internacional"
 */

require_once __DIR__ . '/../../src/config.php';
require_once __DIR__ . '/../../src/db.php';

$file_in = __DIR__ . '/../originais/Ficha Filiação 2020.csv';
$file_out = __DIR__ . '/../limpos/filiados_2020_limpo.csv';

// Mapeamento de normalização
$cidades_map = require __DIR__ . '/cidades_normalizadas.php';

// Mapeamento de categorias (valores de 2020)
$categorias_map = [
...
function extrair_cidade_estado($endereco2, $cidades_map) {
    // Formato esperado: "Cidade, Estado" ou "Cidade, UF"
    ...
    // Normaliza estado (converte nome completo para sigla)
    ...
    // Se estado ainda é longo, tenta extrair sigla
    if (strlen($estado) > 2) {
        // Tenta pegar últimas 2 letras se parecer UF
...
while (($row = fgetcsv($handle_in)) !== false) {
    $data_raw = trim($row[1] ?? '');
    $email_raw = trim($row[2] ?? '');
    $nome_raw = trim($row[3] ?? '');
    $formacao_raw = trim($row[4] ?? '');
    $endereco1_raw = trim($row[6] ?? '');
    $endereco2_raw = trim($row[7] ?? '');
    $telefone_raw = trim($row[8] ?? '');
    $categoria_raw = trim($row[9] ?? '');
    ...
    $cat_info = $categorias_map[$cat_key] ?? ['categoria' => 'profissional_internacional', 'valor' => 29000];
