...
// Só linha de comando. Sem isto, um GET em /scripts/<arquivo> executa o script
// pelo navegador: processar_lembretes.php dispara os emails, enviar_campanha.php
// manda um lote. A .htaccess tambem bloqueia o diretorio, mas ela e configuracao
// do servidor e pode nao existir noutra instalacao.
if (PHP_SAPI !== 'cli') {
    http_response_code(403);
    exit('Este script roda apenas por linha de comando.');
}
...
switch ($comando) {
    ...
    case 'pagar':
        ...
        db_execute("
            UPDATE filiacoes
            SET status = 'pago', metodo = COALESCE(metodo, 'manual'), data_pagamento = ?, status_at = ?
            WHERE id = ?
        ", [date('Y-m-d H:i:s'), date('Y-m-d H:i:s'), $filiacao_id]);
    ...
    case 'novo':
        ...
        if ($fil_existe) {
            db_execute("UPDATE filiacoes SET status = 'pago', metodo = 'manual', categoria = ?, valor = ?, data_pagamento = ?, status_at = ? WHERE id = ?",
                [$categoria, $valor, date('Y-m-d H:i:s'), date('Y-m-d H:i:s'), $fil_existe['id']]);
        } else {
            db_insert("
                INSERT INTO filiacoes (pessoa_id, ano, categoria, valor, status, metodo, data_pagamento, status_at, created_at)
                VALUES (?, ?, ?, ?, 'pago', 'manual', ?, ?, ?)
            ", [$pessoa_id, $ano, $categoria, $valor, date('Y-m-d H:i:s'), date('Y-m-d H:i:s'), date('Y-m-d H:i:s')]);
        }
