...
<article>
        ...
        <?php if ($pix_data && !empty($pix_data['qr_code'])): ?>
            <div class="text-center">
                ...
                <p style="margin-top: 15px; padding: 10px; background: #e8f5e9; border-radius: 6px; color: #2e7d32;">
                    Após o pagamento, você receberá um email de confirmação em até 30 minutos.
                </p>
            </div>
        <?php else: ?>
            <p>Gere um novo código PIX para pagar:</p>
            <form method="POST" action="/filiacao/<?= e($ano) ?>/<?= e($token) ?>/gerar-pix"><?= campo_csrf() ?>
        ...
        <?php if ($boleto_data && !empty($boleto_data['boleto_link'])): ?>
            <div class="text-center">
                ...
                <p style="margin-top: 15px; padding: 10px; background: #fff3e0; border-radius: 6px; color: #e65100;">
                    Por se tratar de boleto bancário, a compensação pode levar até 2 dias úteis. Você receberá um email de confirmação assim que o pagamento for identificado.
                </p>
            </div>

            <form method="POST" action="/filiacao/<?= e($ano) ?>/<?= e($token) ?>/gerar-boleto" style="margin-top: 20px;"><?= campo_csrf() ?>
                <button type="submit" class="secondary">Gerar Novo Boleto</button>
            </form>
        <?php else: ?>
            <p>Gere um boleto para pagar:</p>
            <form method="POST" action="/filiacao/<?= e($ano) ?>/<?= e($token) ?>/gerar-boleto"><?= campo_csrf() ?>
    ...
    <div id="content-cartao" class="tab-content">
        ...
        <form id="form-cartao" method="POST" action="/filiacao/<?= e($ano) ?>/<?= e($token) ?>/pagar-cartao"><?= campo_csrf() ?>
