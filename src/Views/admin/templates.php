<?php
// Servido LOCAL, e nao pelo CDN. Aqui a razao nao e disponibilidade: e que esta
// tela roda dentro da sessao do admin, ao lado de dado pessoal. Script de
// terceiro em tag flutuante (@4) significa que uma publicacao no npm — ou o
// comprometimento do CDN — executa codigo novo na sessao sem ninguem aprovar.
// Atualizar: baixar de cdn.jsdelivr.net/npm/jodit@<versao>/es2021/
?>
...
<article>
    ...
    <details style="margin-bottom: 15px; background: #f8f9fa; padding: 12px 15px; border-radius: 8px; border: 1px solid #dee2e6;" data-tipo="<?= e($tpl['tipo']) ?>">
        ...
        <form method="POST" action="/admin/templates" style="margin-top: 15px;" onsubmit="syncEditor('<?= e($tpl['tipo']) ?>')"><?= campo_csrf() ?>
        ...
        <form method="POST" action="/admin/templates/resetar" style="margin-top: 10px; border-top: 1px solid #dee2e6; padding-top: 10px;"><?= campo_csrf() ?>
            <input type="hidden" name="tipo" value="<?= e($tpl['tipo']) ?>">
