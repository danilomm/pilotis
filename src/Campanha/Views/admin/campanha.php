<article>
    ...
    <div style="border: 2px dashed #28a745; border-radius: 8px; padding: 20px; margin-bottom: 20px; background: #f8fff8;">
        ...
        <form method="POST" action="/admin/campanha/criar" style="display: flex; gap: 15px; align-items: center; flex-wrap: wrap;"><?= campo_csrf() ?>
            <div>
        ...
        <div style="border: 2px solid <?= $border_color ?>; border-radius: 8px; padding: 20px; margin-bottom: 20px;">
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                ...
                <div style="display: flex; gap: 10px; align-items: center;">
                    ...
                    <?php if ($is_aberta && (int)($stats['total'] ?? 0) === 0): ?>
                        <form method="POST" action="/admin/campanha/excluir" style="margin: 0;"><?= campo_csrf() ?>
            ...
            <?php if ($is_aberta): ?>
                ...
                <details style="margin-bottom: 15px; background: #f8f9fa; padding: 12px 15px; border-radius: 8px; border: 1px solid #dee2e6;">
                    ...
                    <form method="POST" action="/admin/campanha/valores" style="margin-top: 12px; display: flex; gap: 10px; flex-wrap: wrap; align-items: flex-end;"><?= campo_csrf() ?>
                        <input type="hidden" name="ano" value="<?= $ano ?>">
                        <div>
                ...
                <!-- Datas de término -->
                <div style="margin-bottom: 15px; background: #f8f9fa; padding: 12px 15px; border-radius: 8px; border: 1px solid #dee2e6;">
                    <form method="POST" action="/admin/campanha/data-fim"><?= campo_csrf() ?>
                        <input type="hidden" name="ano" value="<?= $ano ?>">
                        <div style="display: flex; gap: 20px; flex-wrap: wrap; align-items: flex-end; margin-bottom: 10px;">
                            <div style="flex: 0 0 auto;">
                                <span style="font-size: 12px; display: block; margin-bottom: 4px;"><strong>Internacional:</strong></span>
                                <?php $dfint_val = !empty($c['data_fim_internacional']) ? date('d/m/Y', strtotime($c['data_fim_internacional'])) : ''; ?>
                                <input type="text" name="data_fim_internacional" value="<?= e($dfint_val) ?>" placeholder="dd/mm/aaaa" maxlength="10" style="width: 120px; padding: 4px 8px; margin-bottom: 0;">
                                <?php if (!empty($c['data_fim_internacional'])): ?>
                                    <?php
                                    $dias_int = (int)((strtotime($c['data_fim_internacional']) - strtotime('today')) / 86400);
                                    $cor_int = $dias_int <= 3 ? '#dc3545' : ($dias_int <= 7 ? '#ffc107' : '#28a745');
                                    ?>
                                    <small style="color: <?= $cor_int ?>; display: block; margin-top: 2px;">
                                        <?php if ($dias_int > 0): ?>
                                            <?= $dias_int ?> dia<?= $dias_int > 1 ? 's' : '' ?>
                                        <?php elseif ($dias_int === 0): ?>
                                            encerra hoje
                                        <?php else: ?>
                                            encerrada
                                        <?php endif; ?>
                            ...
                            <div style="flex: 0 0 auto;">
                                <span style="font-size: 12px; display: block; margin-bottom: 4px;"><strong>Nacional/Estudante:</strong></span>
                                <?php $df_val = !empty($c['data_fim']) ? date('d/m/Y', strtotime($c['data_fim'])) : ''; ?>
                                <input type="text" name="data_fim" value="<?= e($df_val) ?>" placeholder="dd/mm/aaaa" maxlength="10" style="width: 120px; padding: 4px 8px; margin-bottom: 0;">
                                <?php if (!empty($c['data_fim'])): ?>
                                    ...
                                    ?>
                                    <small style="color: <?= $cor_dias ?>; display: block; margin-top: 2px;">
                                        <?php if ($dias_restantes > 0): ?>
                                            <?= $dias_restantes ?> dia<?= $dias_restantes > 1 ? 's' : '' ?>
                            ...
                            <button type="submit" style="background: #17a2b8; color: white; border: none; padding: 6px 12px; font-size: 13px; margin-bottom: 0;">Salvar</button>
                        </div>
                    </form>
                </div>
                ...
                <details style="margin-bottom: 15px; background: #fff8e1; padding: 12px 15px; border-radius: 8px; border: 1px solid #ffe082;">
                    ...
                    <form method="POST" action="/admin/campanha/grupo-teste" style="margin-top: 10px;"><?= campo_csrf() ?>
                    ...
                    <form method="POST" action="/admin/campanha/enviar-teste" style="margin-top: 10px;"><?= campo_csrf() ?>
                        <input type="hidden" name="ano" value="<?= $ano ?>">
                ...
                <!-- Iniciar Campanha -->
                <?php $iniciada = $campanha_iniciada[$ano] ?? false; ?>
                <?php if (!$iniciada): ?>
                <div style="margin-bottom: 15px; background: #e3f2fd; padding: 15px; border-radius: 8px; border: 2px solid #1565c0;">
                    <p style="margin: 0 0 10px 0; color: #1565c0;">
                        <strong>Campanha ainda nao iniciada.</strong> Apos testar o grupo de teste, clique abaixo para liberar o envio automatico diario (9h).
                    </p>
                    <form method="POST" action="/admin/campanha/iniciar" style="margin: 0;"><?= campo_csrf() ?>
                        <input type="hidden" name="ano" value="<?= $ano ?>">
                        <button type="submit" style="background: #1565c0; color: white; border: none; padding: 10px 24px; font-size: 15px; border-radius: 5px; cursor: pointer;"
                                onclick="return confirm('Iniciar campanha <?= $ano ?>? O envio automatico diario sera liberado.')">
                            Iniciar Campanha <?= $ano ?>
                        </button>
                    </form>
                </div>
                <?php else: ?>
                <div style="margin-bottom: 15px; background: #e8f5e9; padding: 10px 15px; border-radius: 8px; border: 1px solid #a5d6a7;">
                    <small style="color: #2e7d32;">Campanha iniciada. Envio automatico diario ativo (9h).</small>
                </div>
                <?php endif; ?>
                ...
                <div style="display: flex; gap: 10px; flex-wrap: wrap; align-items: center;">
                    <form method="POST" action="/admin/campanha/fechar" style="margin: 0;"><?= campo_csrf() ?>
                        <input type="hidden" name="ano" value="<?= $ano ?>">
            ...
            <?php else: ?>
                ...
                <?php
                    ...
                    $cad_incompl = (int)($m['cadastros_incompletos'] ?? 0);
                ?>
                ...
                <div style="display: flex; gap: 10px; flex-wrap: wrap; margin-bottom: 15px;">
                    ...
                    <div style="flex: 1; min-width: 120px; background: #f3e5f5; padding: 10px; border-radius: 8px; text-align: center;" title="Pessoas que informaram email mas nao concluiram o formulario. Ficam no banco para mala direta na proxima campanha.">
                        <strong style="color: #6a1b9a;"><?= $cad_incompl ?></strong>
                        <br><small style="color: #6a1b9a;">📝 Cadastros incompletos</small>
                    </div>
                </div>
...
<script>
// Token de CSRF desta sessao: os lotes vao por fetch, e o dispatcher
// exige o token tambem no cabecalho.
...
async function carregarLembretes(ano) {
    try {
        const resp = await fetch('/admin/lembretes/contar', {
            method: 'POST',
            headers: {'Content-Type': 'application/x-www-form-urlencoded', 'X-CSRF-Token': CSRF},
            body: 'ano=' + ano
        });
        ...
        const tipos = {
            ...
            'vencimento_amanha': 'Vencimento amanha',
            'pagamento_vencido': 'Pagamento vencido',
            'ultima_chance': 'Ultima chance',
            'ultima_chance_internacional': 'Ultima chance (internacional)',
        };

        let html = '<table style="width: 100%; font-size: 0.85em; margin-bottom: 5px;">';
        html += '<tr style="border-bottom: 1px solid #ffe0b2;"><th style="text-align: left; padding: 3px 0;">Tipo</th><th style="text-align: right; padding: 3px 0;">Prontos</th><th style="text-align: right; padding: 3px 0;">Enviados</th></tr>';
        for (const [tipo, label] of Object.entries(tipos)) {
            const pendente = data[tipo] || 0;
            const enviado = (data.enviados && data.enviados[tipo]) || 0;
            if (pendente === 0 && enviado === 0) continue;
            const cor = pendente > 0 ? '#e65100' : '#888';
            html += '<tr style="color: ' + cor + ';">'
                + '<td style="padding: 3px 0;">' + label + '</td>'
                + '<td style="text-align: right; padding: 3px 0;">' + (pendente > 0 ? '<strong>' + pendente + '</strong>' : '0') + '</td>'
                + '<td style="text-align: right; padding: 3px 0;">' + enviado + '</td>'
                + '</tr>';
        }
        html += '<tr style="border-top: 1px solid #ffe0b2; font-weight: bold;">'
            + '<td style="padding: 3px 0;">Total</td>'
            + '<td style="text-align: right; padding: 3px 0;">' + data.total + '</td>'
            + '<td style="text-align: right; padding: 3px 0;">' + data.total_enviados + '</td>'
            + '</tr>';
        if (data.agendados_futuro > 0) {
            html += '<tr style="color: #888;"><td colspan="3" style="padding: 3px 0; font-size: 0.9em;">' + data.agendados_futuro + ' agendados para os proximos dias</td></tr>';
        }
        html += '</table>';

        document.getElementById('lembretes-info-' + ano).innerHTML = html;

        // Badge no summary
        const badge = document.getElementById('lembretes-badge-' + ano);
        if (data.total > 0) {
            badge.textContent = data.total + ' pronto' + (data.total > 1 ? 's' : '') + ' para envio';
        } else if (data.agendados_futuro > 0) {
            badge.textContent = data.agendados_futuro + ' agendados';
            badge.style.color = '#888';
        } else {
            badge.textContent = 'Nenhum pendente';
            badge.style.color = '#888';
        }
    } catch (e) {
        document.getElementById('lembretes-info-' + ano).innerHTML = '<small style="color: red;">Erro ao carregar</small>';
    }
}
